...
const tmpBaseDir = process.env.TMPDIR || process.env.TEMP || '/tmp'
const primaryDataDir = configuredDataDir && configuredDataDir.length > 0 ? path.resolve(configuredDataDir) : path.join(process.cwd(), 'data')
const fallbackDataDir = path.join(tmpBaseDir, 'schooltestingapp-data')
